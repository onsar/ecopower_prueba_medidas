...
void buildPowerMessage(uint8_t output){
  ...
  millis_init =micros();
  v_ante = analogRead(ENTER_1);
  
  while (micros()<(millis_init+20000)){
    v_inst = analogRead(ENTER_1);
    v_medi = (v_inst-v_ante)/2;
    v_ante = v_inst;
    i_inst = analogRead(ENTER_7); 
    p_inst = v_medi * i_inst;
    p_suma = p_suma + p_inst;
    medidas++;
  }
  millis_end =micros();

  // Serial.print ("inicio_bucle_lectura: ");          Serial.println(millis_init); 
  // Serial.print ("inicio_bucle_lectura: ");          Serial.println(millis_end);
  Serial.print ("medidas_bucle_lectura: ");         Serial.println(medidas);
